...
# Task 1: Implement Pure Fucntions for Student Management (add_student, delete_student)
...
def find_open_courses(courses):
    '''
    course = [
    {'course_id': 'CS101', 'name': 'Introduction to Computer Science', 'steats_available': '30', 'students': [1,2]}, 
    {'course_id': 'MATH101', 'name': 'Calculuis', 'steats_available': '20', 'students': []},
    {'course_id': 'ENG101', 'name': 'English Composition', 'steats_available': '25', 'students': [3,4]} 
    ]
    '''
...
def get_grades(grades):
    ...
    return map(lambda x: {'student_id': x['student_id'], 
...
def  get_student_average(scores, id):
    ...
    '''
    filtered_scores = [score['points'] for score in scores if score['student_id'] == id]
...
def get_enrolled_students(courses, course_id, students):
    ...
    '''
    [
    {'course_id': 'CS101', 'students': [1.2], 'name': 'Introduction to Computer Science', 'steats_available': '30', 'students': [1,2]}, 
    '''
...
def add_student_to_course(student_id, course_id, courses):
    ...
    if student_id not in students:
        print(f"ErrorL Student with ID {student_id} does not exist")
...
course = [
    {'course_id': 'CS101', 'students': [1.2], 'name': 'Introduction to Computer Science', 'steats_available': '30', 'students': [1,2]}, 
    {'course_id': 'MATH101', 'name': 'Calculuis', 'steats_available': '20', 'students': []},
    {'course_id': 'ENG101', 'name': 'English Composition', 'steats_available': '0', 'students': [3,4]}
]
...
updated_students = add_student(students, {'id': 5, 'ma,e': 'Mary'})
...
student_1_avg = get_student_average(scores, 1)
print("Student 1's Average: ", student_1_avg)

students_in_eng101 = get_engrolled_students(courses, 'ENG101', students)
print("Students in ENG101: ", students_in_eng101)

# Task 5: Apply add_student partially before adding students
partiatl_add_students = partial(add_student, database = students_after_delete)
